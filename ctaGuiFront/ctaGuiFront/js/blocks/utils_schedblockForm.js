...
window.SchedblockForm = function (optIn) {
  ...
  function createTimeInformation () {
    ...
    for (let i = 0; i < label.length; i++) {
      ...
      g.append('rect')
        ...
        .attr('height', box.h) //  Math.min(box.h, line * schedB.blocks.length))
    ...
    enter.each(function (d, i) {
      ...
      g.append('rect')
        ...
        .attr('fill-opacity', (com.schedule.editabled && !stilleEditable) ? 0.7 : 0)
      ...
      function drawTime (id, x, w, y, time) {
        let stock = {}
        let offY = y + ((!com.schedule.editabled || !stilleEditable) ? line * 0.25 : 0)
        let hour = ('0' + d3.timeFormat('%H')(time)).slice(-2)
        let hbox = {
          x: x,
          y: offY,
        ...
        if (!(!com.schedule.editabled || !stilleEditable)) {
        ...
        stock.hour = inputDate(ig,
          hbox,
          'hour',
          {disabled: (!com.schedule.editabled || !stilleEditable), value: hour, min: 0, max: 23, step: 1, 'button-disabled': false},
        ...
        stock.minute = inputDate(ig,
          mbox,
          'minute',
          {disabled: (!com.schedule.editabled || !stilleEditable), value: min, min: 0, max: 60, step: 1, 'button-disabled': false},
        ...
        stock.second = inputDate(ig,
          sbox,
          'second',
          {disabled: (!com.schedule.editabled || !stilleEditable), value: sec, min: 0, max: 60, step: 1, 'button-disabled': false},
      ...
      let sbox = {
        x: label[1].x - 1,
        y: (com.schedule.editabled && !stilleEditable) ? line * 0.15 : 0,
        w: label[1].w + 1,
        h: line
      }
      dropDownDiv(g,
        sbox,
        'state',
        {disabled: (!com.schedule.editabled || !stilleEditable), value: d.exeState.state, options: options},
    ...
    g.append('line')
      .attr('x1', box.x)
      .attr('y1', box.h) // Math.min(box.h, headerSize + 3 + line * schedB.blocks.length))
      .attr('x2', box.w)
      .attr('y2', box.h) // Math.min(box.h, headerSize + 3 + line * schedB.blocks.length))
      .attr('stroke', colorPalette.dark.stroke)
  ...
  function dropDownDiv (g, box, id, optIn, events) {
    ...
    let selector = rootDiv.append('select')
      .style('width', '100%')
      .style('height', '100%')
      .style('border', 'inset 1px solid #000000')
