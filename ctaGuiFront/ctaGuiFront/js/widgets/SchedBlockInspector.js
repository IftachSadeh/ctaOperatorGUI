...
let main_sched_blocksInspector = function (opt_in) {
  ...
  function init_data (data_in) {
    ...
    function initDefaultStyle () {
      ...
      shared.style.blockCol = function (opt_in) {
        ...
        //   ? opt_in.endTime
        //   : undefined
        // if (endTime < Number(shared.data.server.time_of_night.now)) return color_theme.blocks.shutdown
  ...
  let focusManager = new function () {
    function unfocusCore (type, id) {
      ...
      if (type === 'sched_block') sched_block()
      if (type === 'block') block()
  ...
  function navigateHistory (mode) {
    if (mode === 'backward') {
      ...
      if (shared.history.index === -1) shared.data.copy.blocks = deep_copy(get_blocksData('server'))
      else shared.data.copy.blocks = deep_copy(shared.history.list[shared.history.index].data)
  ...
  function setCol (opt_in) {
    if (opt_in.endTime < Number(shared.data.server.time_of_night.now)) return color_theme.blocks.shutdown
    ...
    function core (blocks) {
      for (let i = 0; i < blocks.length; i++) {
        ...
        if (!temp[b.sched_block_id]) {
          ...
          sb.blocks.push(b)
          if (b.exe_state.state === 'run' || sb.exe_state.state === 'run') sb.exe_state.state = 'run'
          else if (b.exe_state.state === 'wait' && sb.exe_state.state === 'done') sb.exe_state.state = 'run'
          else if (b.exe_state.state === 'done' && sb.exe_state.state === 'wait') sb.exe_state.state = 'run'
        }
      }
    }
  ...
  function optimizer () {
      ...
      for (let i = b1.tel_ids.length - 1; i > -1; i--) {
        for (let j = b2.tel_ids.length - 1; j > -1; j--) {
          if (b1.tel_ids[i] === b2.tel_ids[j]) {
            if (Math.random() > 0.5) remTels1.push(b1.tel_ids.splice(i, 1)[0])
            else remTels2.push(b2.tel_ids.splice(j, 1)[0])
            break
          }
        }
      }
    ...
    for (let i = shared.data.copy[shared.data.current].optimized.blocks.wait.length - 1; i > -1; i--) {
      ...
      for (let j = 0; j < shared.data.copy[shared.data.current].optimized.blocks.run.length; j++) {
        ...
        if (is_same_time(mb.startTime, mb.endTime, tb.startTime, tb.endTime)) {
          shareSameTels(mb, tb)
        }
      }
  ...
  function createModificationsList () {
    ...
    for (let key in shared.data.copy.blocks) {
      for (let i = 0; i < shared.data.copy.blocks[key].length; i++) {
        ...
        if (sched.length === 0) {
          if (diff.length !== 0) shared.data.copy.modifications.push({id: block.sched_block_id, name: block.metadata.n_sched, blocks: [block]})
        } else {
  ...
  function createSchedName (blocks) {
    ...
    for (let key in blocks) {
      for (let i = 0; i < blocks[key].length; i++) {
        if (blocks[key][i].metadata.n_sched > max) max = blocks[key][i].metadata.n_sched
      }
    }
    return max + 1
  }
  ...
  function updateBlockState (block, newState) {
    if (block.exe_state.state === newState) return
  ...
  let Svg_blocks_queue_server = function () {
      ...
      blockQueue.switchStyle({
        ...
        blockPattern: function (opt_in) {
          let startT = is_def(opt_in.startTime)
            ? opt_in.startTime
            : opt_in.d.startTime
          if (startT < shared.data.server.time_of_night.now) return 'url(#patternLock)'
          return 'none'
        }
      })
    ...
    function update_data () {
      ...
      if (newWidth > box.block_queue_server.w) newWidth = box.block_queue_server.w
  ...
  let SvgTargets = function () {
    ...
    function showPercentTarget (block) {
      ...
      let target = reserved.clipping.clipBody.selectAll('g.target')
        .filter(function (d) { return (block.target_id === d.id) }).select('path')._groups[0][0]
      let scaleX = d3.scaleLinear()
        .range([0, reserved.box.w])
    ...
    function computeTelsCurve (block) {
      ...
      for (let key in focusBlockList) {
        for (let i = 0; i < focusBlockList[key].length; i++) {
          ...
          if (b.exe_state.state === 'cancel') continue
          if (block && b.obs_block_id === block.obs_block_id) continue
  ...
  let SvgFocusOverlay = function () {
    ...
    function overBlock (id) {
      if (shared.focus) unfocusOn_block(shared.focus.id)
      let d = getBlockById(get_blocksData(), id).data
      show_block_info(d)
    }
    ...
    function outBlock (id) {
      let d = getBlockById(get_blocksData(), id).data
      hide_block_info(d)
      if (shared.focus) focusOn_block(shared.focus.id)
    }
    this.outBlock = outBlock

    function canDrag (d) {
      if (d.created) return true
      if (d.exe_state.state === 'wait' || d.exe_state.state === 'cancel') return true
      return false
    }
    function dragStart (d) {
      if (!canDrag(d)) return

      if (!reserved.drag) focusManager.focusOn('block', d.obs_block_id)
    ...
    function dragTick (d) {
      if (!canDrag(d)) return
      if (d.exe_state.state === 'run') return

      reserved.drag.atLeastOneTick = true

      if (d3.event.dx < 0 &&
        Math.floor(reserved.drag.timescale.invert(reserved.drag.position.left + d3.event.dx)) < Number(shared.data.server.time_of_night.now)) return
      ...
      if (!shared.focus || shared.focus.id !== d.obs_block_id) focusManager.focusOn('block', d.obs_block_id)
    ...
    function dragEnd (d) {
      ...
      // if (d.endTime < Number(shared.data.server.time_of_night.now)) return
      //
    ...
    for (let j = conflict_button.length - 1; j >= 0; j--) {
      ...
      for (let i = 0; i < linked.length; i++) {
        linked[i].d3
          ...
          .on('mouseover', function (d) {
            ...
            if (conflict_button[j].d3) conflict_button[j].d3.select('rect').attr('fill', colorPalette.darkest.background)
          ...
          .on('mouseout', function (d) {
            ...
            if (conflict_button[j].d3) conflict_button[j].d3.select('rect').attr('fill', colorPalette.darker.background)
      ...
      if (conflict_button[j].d3) {
        conflict_button[j].d3
          ...
          .on('mouseout', function (d) {
            d3.select(this).style('cursor', 'default')
            if (conflictFocused.d === conflict_button[j].d) return
            blockQueue.highlightBlocks([])
  ...
  let SvgRight_info = function () {
    ...
    function createBlocks_info_panel (idBlock) {
      ...
      reserved.obsblockForm = new ObsblockForm({
        ...
        events: {
          allTel: function () { return {allTels: shared.data.server.inst_health, blocks: get_blocksByTime(get_blocksData(), data.time.start, data.time.end)} },
          blurry: function () {
            svg_events_queue_server.blurry()
            svg_blocks_queue_server.blurry()
            svgBrush.blurry()
