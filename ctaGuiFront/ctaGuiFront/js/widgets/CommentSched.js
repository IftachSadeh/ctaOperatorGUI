  ...
  function extractTargets () {
    ...
    for (let key in shared.data.server.blocks) {
      ...
      for (let i = 0; i < arr.length; i++) {
        if (t.indexOf(arr[i].target_id) === -1) t.push(arr[i].target_id)
      }
    }
    return t
  }
  ...
  let Svg_blocks_queue_server = function () {
    ...
    function init_data () {
      ...
      reserved.icon.g
        ...
        .attr('stroke-width', 0.2)
        .on('mouseover', function () {
          if (reserved.main.mode === 'expand') { d3.select(this).attr('fill', color_theme.bright.background) } else d3.select(this).attr('fill', color_theme.darker.background)
        })
        .on('mouseout', function () {
          if (reserved.main.mode === 'expand') { d3.select(this).attr('fill', color_theme.darker.background) } else d3.select(this).attr('fill', color_theme.bright.background)
        })
        .on('click', function () {
          let dataPointFuturTop = [
            { x: -4, y: 4 },
