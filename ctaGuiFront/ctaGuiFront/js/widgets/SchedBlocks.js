...
function main_sched_blocks (opt_in) {
  ...
  function sortBlocksByState (array) {
    ...
    for (var i = 0; i < array.length; i++) {
      let b = array[i]
      if (b.exe_state.state === 'done') blocks.success.push(b)
      else if (b.exe_state.state === 'fail') blocks.fail.push(b)
      else if (b.exe_state.state === 'cancel') blocks.cancel.push(b)
      else if (b.exe_state.state === 'wait') blocks.wait.push(b)
    }
    return blocks
  }
  ...
  let SvgBrushPast = function () {
    ...
    function update_data () {
      let date = new Date(shared.data.server.time_of_night.date_now)
      let startTime = {date: new Date(shared.data.server.time_of_night.date_now).setSeconds(date.getSeconds() - (3600 * 8)), time: Number(shared.data.server.time_of_night.now) - (3600 * 8)}
      let endTime = {date: new Date(shared.data.server.time_of_night.date_now).setSeconds(date.getSeconds()), time: Number(shared.data.server.time_of_night.now)}
  ...
  let SvgRunningPhase = function () {
    ...
    function update_data () {
      ...
      if (totHeight > 1) {
        ratio = 1 / totHeight
        totHeight = 1
      } else if (totHeight < 0.25) offset_runningBlocks = 0.3
      else if (totHeight < 0.50) offset_runningBlocks = 0.2
      else if (totHeight < 0.75) offset_runningBlocks = 0.1
      ...
      function initRunPhase (g, run_phase, gt) {
        if (run_phase.length < 1) return
        if (run_phase[0].includes('config')) initConfig(g, run_phase, gt)
        if (run_phase[0].includes('takeData')) initTake(g, run_phase, gt)
        if (run_phase[0].includes('finish')) initFinish(g, run_phase, gt)
      }
      function initConfig (g, run_phase, gt) {
        ...
        for (let i = 0; i < run_phase.length; i++) {
          if (run_phase[0].includes('mount')) g.select('#mount').attr('fill', colorFree) // .attr('stroke-width', 0.2)
          if (run_phase[0].includes('camera')) g.select('#camera').attr('fill', colorFree) // .attr('stroke-width', 0.2)
          if (run_phase[0].includes('mirror')) g.select('#mirror').attr('fill', colorFree) // .attr('stroke-width', 0.2)
          if (run_phase[0].includes('Daq')) g.select('#daq').attr('fill', colorFree) // .attr('stroke-width', 0.2)
        }
      }
      ...
      function initFinish (g, run_phase, gt) {
        ...
        for (let i = 0; i < run_phase.length; i++) {
          if (run_phase[0].includes('mount')) g.select('#mount').attr('fill', colorLock) // .attr('stroke-width', 0.2)
          if (run_phase[0].includes('camera')) g.select('#camera').attr('fill', colorLock) // .attr('stroke-width', 0.2)
          if (run_phase[0].includes('mirror')) g.select('#mirror').attr('fill', colorLock) // .attr('stroke-width', 0.2)
          if (run_phase[0].includes('Daq')) g.select('#daq').attr('fill', colorLock) // .attr('stroke-width', 0.2)
        }
      }
  ...
  let SvgFreeTels = function () {
    ...
    function update_data () {
      ...
      for (let i = 0; i < shared.data.server.tel_ids.length; i++) {
        ...
        tels.push({id: id, health: get_tel_healthById(id)})
      }
  ...
  function getFocusBlock (opt_in) {
    ...
    if (is_def(blocks)) {
      $.each(blocks, function (index0, data_now0) {
        if (is_def(focusBlock)) return
        $.each(data_now0, function (index1, data_now1) {
          if (opt_in.focus_id === data_now1.obs_block_id) focusBlock = data_now1
        })
      })
    }
