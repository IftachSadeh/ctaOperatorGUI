  ...
  function update (data_in) {
    ...
    if (isNewHirch) {
      ...
      let hirch = d3
        ...
        .sum(function (d) {
          if (d.nTel === -1 || is_def(d.children)) return 2
          else return 1
        })
        .sort(function (a, b) {
      ...
      text
        ...
        // .text(com.style.text)
        .text(function (d, i) {
          if (d.data.nTel === -1) return d.data.block_name
          else return tel_info.get_title(d.data.id)
        })
  ...
  function setStyle (opt_in) {
    ...
    com.style.fill = function (d, n_obs) {
      if (d.height === 0) {
        ...
        } else return inst_health_col(com.inst_health[d.data.id])
      } else {
        return 'transparent'
      }
    }
      ...
      if (d.height === 0) {
        if (d.data.nTel === -1) {
          return d3
            ...
            .darker(1.0)
        } else return inst_health_col(com.inst_health[d.data.id], 0.5)
      } else if (d.height === 1) {
        return d3
          .rgb(com.recCol({ d: d, state: 'run', n_obs: n_obs, can_run: true }))
          .darker(1.0)
