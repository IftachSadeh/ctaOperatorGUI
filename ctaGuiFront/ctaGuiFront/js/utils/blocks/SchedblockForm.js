...
window.SchedblockForm = function (opt_in) {
  ...
  function createTime_information () {
    ...
    enter.each(function (d, i) {
      ...
      if (d.exe_state.state === 'wait') options = ['cancel', 'wait']
      if (d.exe_state.state === 'cancel') options = ['cancel', 'wait']
      if (d.exe_state.state === 'run') options = ['run', 'cancel']
  ...
  function createPointing_information () {
    ...
    function pointingCore (block, pg, offset) {
      ...
      enter.each(function (d, i) {
        let g = d3.select(this)
        let pevents = {
          click: function () { com.schedule.events.click('block', block.obs_block_id) },
          over: function () {},
          out: function () {}
        }
