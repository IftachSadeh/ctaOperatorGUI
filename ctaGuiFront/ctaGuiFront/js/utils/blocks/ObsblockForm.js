...
window.ObsblockForm = function (opt_in) {
  ...
  function changeBlockTime (type, hour, min, sec) {
    let startTime = new Date(com.data.time_of_night.date_start)
    ...
    startTime = new Date(com.data.time_of_night.date_start)
  ...
  function initTime_information () {
    ...
    let options = []
    if (data.exe_state.state === 'wait') options = ['cancel', 'wait']
    if (data.exe_state.state === 'cancel') options = ['cancel', 'wait']
    if (data.exe_state.state === 'run') options = ['run', 'cancel']
    ...
    drawTime('startTime', label[1].x + txt_size * 0.5, label[1].w, 2 + headerSize * 1.5, startTime)
    drawTime('endTime', label[2].x + txt_size * 0.5, label[2].w, 2 + headerSize * 1.5, endTime)
  ...
  function switchTel (elem, tel) {
    ...
    let trueBlock = []
    allTel.allTels = allTel.allTels.filter(function (d) { return com.data.block.tel_ids.indexOf(d.id) === -1 })
    ...
    function duplicateTels () {
      for (let i = 0; i < trueBlock.length; i++) {
        ...
        for (let z = b1.tel_ids.length - 1; z >= 0; z--) {
          ...
          if (dupli) newBlock.tel_ids.push(b1.tel_ids.splice(z, 1)[0])
        }
        if (newBlock.tel_ids.length > 0) trueBlock.push(newBlock)
      }
    }
    ...
    g.append('rect')
      ...
      .on('click', function () {
        ...
        for (let i = 0; i < trueBlockReference.length; i++) {
          if (trueBlockReference[i].tel_ids.indexOf(choosenTel.id) !== -1) removeTelescopeFromBlock(trueBlockReference[i], choosenTel)
        }
  ...
  function initTelescope_information () {
    ...
    function createInput (tel_type, g, innerbox) {
      ...
      com.telescope.tels[tel_type] = input_number_d3(g,
        ...
        {change: (d) => { changeTelescopeNumber(tel_type, d) }, enter: (d) => { changeTelescopeNumber(tel_type, d) }})
