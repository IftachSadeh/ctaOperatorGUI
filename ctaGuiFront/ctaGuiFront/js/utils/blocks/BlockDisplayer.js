...
window.BlockDisplayer = function (opt_in) {
  ...
  function setDefaultStyle () {
    ...
    com.style.blockCol = function (opt_in) {
      ...
      if (state === 'wait') {
        ...
        if (is_def(can_run)) {
          if (!can_run) return com.blockQueue.blocks.colorPalette.cancelOp
        }
  ...
  let BlockQueueBib = function () {
    ...
    function calcBlockRow (opt_in) {
      ...
      $.each(data_in, function (index, data_now) {
        ...
        let end = (data_now.time.endTime - opt_in.start) * xScale
        let overlap = (data_now.time.endTime - data_now.time.start) * xScale * 0.2 // allow small overlap in x between blocks
        let x0 = box.x + start
        let w0 = end - start
      ...
      rect.each(function (d, i) {
        ...
        d3.select(this).select('text')
          ...
          .style('font-size', function (d) {
            ...
            if (!is_def(d.display.size)) d.display.size = 0
  ...
  let BlockQueue2Bib = function () {
    ...
    function get_line_layout () {
      let scheds = groupBlocksBySchedule(com.data.filtered)
      lineCount(scheds)
      scheds.forEach(d => (d.blocks = d.blocks.map(function (dd) { return {obs_block_id: dd.obs_block_id, nLine: dd.nLine} })))
      return scheds
    }
      ...
      enterAllScheds.each(function (d, i) {
        if (com.blockTrackShrink.sched_blocks.label.enabled) {
          ...
          d3.select(this).append('text')
            ...
            .text('S' + d.name)
            .attr('x', function () {
              if (com.blockTrackShrink.sched_blocks.label.position === 'left') return (timescale(d.startT)) - 5
              else if (com.blockTrackShrink.sched_blocks.label.position === 'right') return (timescale(d.endT)) + 5
      ...
      enterAllScheds.merge(allScheds)
        .each(function (d, i) {
          ...
          d3.select(this).select('text#schedId')
            ...
            .attr('text-anchor', function () {
              if (com.blockTrackShrink.sched_blocks.label.position === 'left') return 'end'
              else if (com.blockTrackShrink.sched_blocks.label.position === 'right') return 'start'
              // if (d.startT < com.time.startTime.time) {
              //   return 'start'
              // } else if (d.endT > com.time.endTime.time) {
              //   return 'end'
  ...
  function filterData (opt_in) {
    ...
    for (var z = 0; z < com.data.raw.blocks.done.length; z++) {
      ...
      if (filters.length === 0) {
        if (data_now.exe_state.state === 'done') filtered.done.push(data_now)
        if (data_now.exe_state.state === 'fail') filtered.fail.push(data_now)
        if (data_now.exe_state.state === 'cancel') filtered.cancel.push(data_now)
      } else {
        for (var i = 0; i < filters.length; i++) {
          let filterNow = filters[i]
          let allPropChecked = true
          if (!checkFilter(data_now, filterNow)) allPropChecked = false
          if (allPropChecked) {
      ...
      if (data_now.filtered) stats.filtered += 1
    }

    filtered.wait = com.data.raw.blocks.wait.map(function (data_now) {
      data_now.filtered = false
      if (filters.length === 0) return data_now
      for (var i = 0; i < filters.length; i++) {
        let filterNow = filters[i]
        let allPropChecked = true
        if (!checkFilter(data_now, filterNow)) allPropChecked = false
