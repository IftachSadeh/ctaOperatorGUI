...
window.tel_info = function () {
  ...
  this.sort_ids = function (opt_in) {
    ...
    if (is_def(opt_in.is_inplace) && !opt_in.is_inplace) data = deep_copy(data)
    ...
    data.sort(function (a, b) {
      ...
      else if (is_ascend) return func(a) < func(b) ? -1 : 1
  ...
  this.get_title = function (id) {
    if (is_def(title[id])) return title[id]
    else {
...
window.inst_health_col = function (health, index) {
  ...
  if (!is_def(index)) return tel_state_color[telState][0]
  else return d3.rgb(tel_state_color[telState][0]).darker(index)
}
...
window.pin_drawer = function (id_layout, id_drawer) {
  ...
  if (ele_layout.force_narrow) tog_drawer_with_pin(ele_layout, ele_drawer)
...
window.move_node_up = function (node, n_levels_up, stop_name) {
  ...
  if (n_levels_up === undefined) n_levels_up = 1e3
  else n_levels_up = Math.min(1e3, Math.max(1, n_levels_up))
  if (stop_name === undefined) stop_name = 'svg'
...
window.get_node_width_by_id = function (opt_in) {
  ...
  if (is_def(ele_now)) return ele_now.getBBox().width
  else return 0
}

window.get_node_height_by_id = function (opt_in) {
  var ele_now = get_node_id(opt_in)
  if (!is_def(opt_in.txt_scale)) opt_in.txt_scale = false
  var txt_scale = opt_in.txt_scale ? get_txt_scale() : 1

  if (is_def(ele_now)) return ele_now.getBBox().height * txt_scale
  else return 0
}
...
window.load_script = function (opt_in) {
  ...
  if (loaded_scripts.queued.indexOf(opt_in.script) < 0) {
    ...
    if (debug) console.log('--- queue ( from', opt_in.source, '):', opt_in.script)

    $.getScript(opt_in.script, function () {
      loaded_scripts.loaded.push(opt_in.script)
      if (debug) console.log('-+- loaded: ', opt_in.script)
    })
  }
}
...
window.Locker = function (opt_init) {
  ...
  function add (opt_in) {
    ...
    if (is_def(opt_in)) {
      if (typeof opt_in === 'string') id = opt_in
        ...
        if (is_def(opt_in.expire)) expire = opt_in.expire
        if (is_def(opt_in.override)) override = opt_in.override
      }
    }

    if (!is_def(counters[id])) counters[id] = 0
  ...
  function remove (opt_in) {
    ...
    if (is_def(opt_in)) {
      if (typeof opt_in === 'string') id = opt_in
        ...
        if (is_def(opt_in.id)) id = opt_in.id
        if (is_def(opt_in.override)) override = opt_in.override
        if (is_def(opt_in.delay)) delay = opt_in.delay
      }
    }

    if (delay <= 0) del_counter()
    else {
      setTimeout(function () {
        del_counter()
      }, delay)
    ...
    function del_counter () {
      if (!is_def(counters[id])) counters[id] = 0 // just in case - should not happen...
  ...
  function is_free (opt_in) {
    ...
    if (is_def(opt_in)) {
      if (typeof opt_in === 'string') id = opt_in
      else if (is_def(opt_in.id)) id = opt_in.id
    }

    if (!is_def(counters[id])) return true
    else {
  ...
  function n_active (opt_in) {
    ...
    if (!is_def(counters[id])) return 0
    else {
      counters[id] = Math.max(0, counters[id])
      return counters[id]
    }
...
window.IconBadge = function () {
  function set_widget_icon (opt_in) {
    ...
    if (!is_def(opt_in.icon_div)) return
    ...
    if (!is_def(icon_div_id_in) || !is_def(n_icon) || n_icon < 0) return
    ...
    if (is_def(icon_div_ele)) {
      ...
      if (!(icon_div_id.indexOf('#') === 0)) icon_div_id = '#' + icon_div_id
    }
  ...
  function add (opt_in) {
    ...
    if (trans_back) {
      if (!is_def(opt_in.col_back)) col_back = 'transparent'
      if (!is_def(opt_in.col_light)) col_light = '#383b42'
      if (!is_def(opt_in.col_light_opac)) col_light_opac = 0.2
    }
    // duration*=10
    ...
    var set_r = function (r_in, duration) {
      if (!is_def(duration)) duration = times.anim_arc
      var trans = -r_in
...
window.bck_pattern = function (opt_in) {
  ...
  if (is_def(texture_orient) || is_def(circ_type)) {
    ...
    if (
      ...
      if (is_def(texture_orient)) com[tag_now].orient = texture_orient
      if (is_def(texture_size)) com[tag_now].size = texture_size
      ...
      if (is_def(size)) com[tag_now].txtr.size(size)
...
window.do_zoom_to_target = function (opt_in) {
  ...
  if (is_def(opt_in.func_start)) opt_in.func_start()
  ...
  function tween_step (t) {
    if(opt_in.is_static) return ''
    
    if (is_def(opt_in.func_during)) opt_in.func_during()

    var intpr = interpol_zoom(t)
    if (!is_def(intpr[0])) return ''
...
window.deep_copy = function (obj_in) {
  if (Array.isArray(obj_in)) return obj_in.slice(0, obj_in.length)
  else return jQuery.extend(true, {}, obj_in)
}
...
window.find_dict_ele_in_obj = function (obj_in, key, val, is_debug) {
  ...
  $.each(obj_in, function (index, ele_now) {
    if (ele_now[key] === val && data_out[0] === -1) data_out = [index, ele_now]
  })
...
window.is_def = function (data_in) {
  ...
  else if (data_in === null) return false
  else if (data_in === Infinity) return false
  else return true
}
...
window.azim_ra = function (data_in) {
  ...
  data_out += 60
  if (data_out > 180) data_out -= 360
  else if (data_out < -180) data_out += 360 // for debug
  return data_out
}
