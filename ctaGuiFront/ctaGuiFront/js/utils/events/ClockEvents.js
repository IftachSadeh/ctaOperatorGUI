...
window.ClockEvents = function() {
        ...
        return (d3.timeSecond(date) < date ? formatMillisecond
            : d3.timeMinute(date) < date ? formatSecond
                : d3.timeHour(date) < date ? formatMinute
                    : d3.timesay(date) < date ? formatHour
    ...
    function getMouseCursorTime(pos) {
        if (pos === null) {
            com.send_function(null)
        }
        else {
            com.send_function(com.axis.scaleX.invert(pos))
        }
    }
    ...
    function init(opt_in) {
        com.g = opt_in.g
        com.box = opt_in.box
        com.color_theme = opt_in.color_theme
        ...
        com.g.append('rect')
            ...
            .attr('fill', com.color_theme.background)
        ...
        com.axis.axisG.select('path').attr('stroke-width', 1).attr('stroke', com.color_theme.stroke)
        ...
        com.shrink_button = com.g.append('g')
        ...
        com.g.append('rect')
            ...
            .attr('height', com.box.h * 0.98)
            .attr('fill', com.color_theme.background)
            .attr('stroke', com.color_theme.stroke)
            .attr('stroke-width', 1)
            .attr('stroke-dasharray', [ (com.box.w * 0.12) + (com.box.h * 0.98), (com.box.w * 0.12) + (com.box.h * 0.98) ])
    ...
    function changeDomainHourBack() {
        ...
        com.axis.axisG
            ...
            .duration(1000)
            .call(com.axis.bottom)
        com.axis.axisG.selectAll('g.tick').selectAll('line').attr('stroke-width', 1).attr('stroke', com.color_theme.stroke)
    ...
    function changeDomainHourFront() {
        com.shrink_button.append('rect')
            ...
            .attr('stroke', com.color_theme.stroke)
            .attr('stroke-width', 1)
            .attr('stroke-dasharray', [ (com.box.w * 0.1), (com.box.h * 0.98) + (com.box.w * 0.1), (com.box.h * 0.98) ])

        let timerange = com.timeRange.total
        let newtime = Math.floor(timerange / 86400)
        com.shrink_button.append('text')
            .attr('class', 'day')
        ...
        com.shrink_button.append('rect')
            ...
            .on('mouseover', function() {
                com.shrink_button.select('text.day')
                    .attr('stroke', '#ffffff')
                    .attr('fill', '#ffffff')
            })
            ...
            .on('wheel', function(d) {
                ...
                if (direction === 'down') {
                    ...
                    if (com.timeRange.day < 0) {
                        com.timeRange.day = 8553600
                    }
                }
                if (direction === 'up') {
                    com.timeRange.day += 86400
                    if (com.timeRange.day > 8553600) {
                        com.timeRange.day = 0
                    }
                }
                let temp = Math.floor(com.timeRange.day / 86400)
                com.shrink_button.select('text.day').text((temp < 10 ? '0' + temp : temp) + ' :')
        ...
        com.shrink_button.append('rect')
            ...
            .attr('opacity', 0)
            .on('mouseover', function() {
                com.shrink_button.select('text.hour')
                    .attr('stroke', '#ffffff')
                    .attr('fill', '#ffffff')
            })
            .on('mouseout', function() {
            ...
            .on('wheel', function(d) {
                ...
                if (direction === 'down') {
                    ...
                    if (com.timeRange.hour < 0) {
                        com.timeRange.hour = 82800
                    }
                }
                if (direction === 'up') {
                    com.timeRange.hour += 3600
                    if (com.timeRange.hour > 82800) {
                        com.timeRange.hour = 0
                    }
                }
                let temp = Math.floor(com.timeRange.hour / 3600)
                com.shrink_button.select('text.hour').text((temp < 10 ? '0' + temp : temp) + ' :')
                changeDomainHourBack()
            })
        com.shrink_button.append('rect')
            ...
            .attr('opacity', 0)
            .on('mouseover', function() {
                com.shrink_button.select('text.minute')
                    .attr('stroke', '#ffffff')
                    .attr('fill', '#ffffff')
            })
            .on('mouseout', function() {
            ...
            .on('wheel', function(d) {
                ...
                if (direction === 'down') {
                    ...
                    if (com.timeRange.minute < 0) {
                        com.timeRange.minute = 3540
                    }
                }
                if (direction === 'up') {
                    com.timeRange.minute += 60
                    if (com.timeRange.minute > 3540) {
                        com.timeRange.minute = 0
                    }
                }
                let temp = Math.floor(com.timeRange.minute / 60)
                com.shrink_button.select('text.minute').text((temp < 10 ? '0' + temp : temp))
                changeDomainHourBack()
            })
    ...
    function drawEvents() {
        for (let i = com.futurEvents.length - 1; i >= 0; i--) {
            if (new Date(com.futurEvents[i].start_date) < com.limitDate) {
                com.timelineEvents.push(com.futurEvents.splice(i, 1)[0])
            }
        }
        com.shrink_button.select('text.futurEvent').text('+' + com.futurEvents.length)

        for (let i = com.timelineEvents.length - 1; i >= 0; i--) {
            if (new Date(com.timelineEvents[i].start_date) < com.currentDate) {
                com.popupEvents.push(com.timelineEvents.splice(i, 1)[0])
            }
        }
        ...
        enterG.each(function(d, i) {
            let rectBack = enterG.select('g.popupG').append('rect')
                ...
                .style('fill', function(d, i) {
                    return com.color_theme.background
                })
    ...
    function drawTimelineEvents() {
        ...
        enterG.each(function(d, i) {
            ...
            d3.select(this).append('rect')
                ...
                .attr('y', function() {
                    return -(com.box.h * 0.1) * (d.pos)
                })
                .attr('width', function() {
                    if (d.end_date) {
                        return (com.axis.scaleX(new Date(d.end_date)) - com.axis.scaleX(new Date(d.start_date)))
                    }
                    else {
                        return 0
                    }
                })
                .attr('height', com.box.h * 0.1)
                .attr('stroke', '#000000')
        ...
        enterG.append('svg:image')
            ...
            .attr('width', com.box.h * 0.5)
            .attr('height', com.box.h * 0.5)
            .attr('x', function(d, i) {
                return com.axis.scaleX(new Date(d.start_date)) - (com.box.h * 0.5 * 0.5)
            })
            .attr('y', function(d, i) {
                return -(com.box.h * 0.6) - (com.box.h * 0.5 * 0.5)
            })
        // enterG.append('rect')
        //   .attr('class', 'icon')
        //   .attr('x', function (d, i) {
