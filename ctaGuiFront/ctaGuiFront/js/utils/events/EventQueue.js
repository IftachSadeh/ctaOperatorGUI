...
window.EventQueue = function () {
  ...
  function initFilters () {
    ...
    function create_button (position, type, priority, filter) {
      ...
      if (type === 'Alarm') drawAlarm(new_button, 0, 0, 16, 16)
      if (type === 'GRB') drawGrb(new_button, 0, 0, 16, 16)
      if (type === 'Hardw.') drawHardware(new_button, 0, 0, 16, 16)
  ...
  function setStyle (opt_in) {
    ...
    if (!is_def(com.style.recCol)) {
      com.style.recCol = function (opt_in) {
        ...
        else if (state === 'done') return d3.color(cols_greens[0]).brighter()
        else if (state === 'run') {
          return d3.color(cols_purples_blues[0]).brighter() // [n_obs % cols_purples_blues.length]
        } else if (state === 'cancel') {
          if (is_def(can_run)) {
            if (!can_run) return d3.color(cols_purples[3]).brighter()
          }
          return d3.color(cols_purples[4])
        } else if (state === 'fail') return d3.color(cols_reds[3]).brighter()
        else return d3.color(col_prime).brighter()
      }
    }
