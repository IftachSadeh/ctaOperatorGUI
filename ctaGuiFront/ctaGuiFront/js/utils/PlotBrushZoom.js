...
window.PlotBrushZoom = function (opt_in) {
  ...
  this.set = function (opt_in) {
    if (is_def(opt_in.data)) reserved[opt_in.tag] = opt_in.data
    else if (is_def(opt_in.def)) reserved[opt_in.tag] = opt_in.def
    else reserved[opt_in.tag] = null
  }
  ...
  function update_data (opt_in) {
    return
    if (opt_in.axis) {
      if (opt_in.axis.top) updateAxis('top', opt_in.axis.top)
      if (opt_in.axis.bottom) updateAxis('bottom', opt_in.axis.bottom)
    }
    updateBrush()
  }
