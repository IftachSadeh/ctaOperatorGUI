...
window.PlotBrushZoom = function (optIn) {
  ...
  //     //   let isSame0 = reserved.brush.domain[0] - reserved.brush.scale.x.domain()[0] <= 0
  //     //   let isSame1 = reserved.brush.domain[1] - reserved.brush.scale.x.domain()[1] >= 0
  //     //   if (isSame0 && isSame1) reserved.brush.domain = null
  //     // }
  //     // updateTopAxis()
  //
  ...
  function initZoom () {
    ...
    //   // )
    //
    //   // reserved.brush.inUserZoom = hasVar(d3.event.sourceEvent)
    //
    //   // if (locker.isFreeV(lockerV.zoomDuring)) {
    //   //   locker.add({ id: lockerZoom.all, override: true })
    //   //   locker.add({ id: lockerZoom.during, override: true })
    ...
    reserved.content.main.g
      .on('wheel', function () {
        ...
        if (d3.event.ctrlKey) {
          ...
          if (zoom.ky.now < zoom.ky.min) zoom.ky.now = zoom.ky.min
          if (zoom.ky.now > zoom.ky.max) zoom.ky.now = zoom.ky.max
          computeZoomFactorky()
        } else {
          ...
          if (zoom.kx.now < zoom.kx.min) zoom.kx.now = zoom.kx.min
          if (zoom.kx.now > zoom.kx.max) zoom.kx.now = zoom.kx.max
          computeZoomFactorkx()
        }
  ...
  function updateData (optIn) {
    return
    if (optIn.axis) {
      if (optIn.axis.top) updateAxis('top', optIn.axis.top)
      if (optIn.axis.bottom) updateAxis('bottom', optIn.axis.bottom)
    }
    updateBrush()
  }
