  ...
  function createAssociatedBlocks () {
    ...
    for (let i = 0; i < scheds.length; i++) {
      for (let j = 0; j < scheds[i].targets.length; j++) {
        ...
        if (!already) targs.push(scheds[i].targets[j])
      }
    }
    ...
    let label = [
      ...
      {x: box.w * 0.15, y: 3 + headerSize * 0.5 + txtSize * 0.3, w: box.w * 0.85, text: 'Scheduling & observing blocks', anchor: 'middle'}
    ]
    for (let i = 0; i < label.length; i++) {
      let off = label[i].anchor === 'middle' ? label[i].w * 0.5 : (label[i].anchor === 'end' ? label[i].w * 0.5 : 0)
      g.append('text')
    ...
    function blockCore (blocks, pg, target) {
      ...
      enter.each(function (d, i) {
        ...
        g.append('text')
          ...
          .style('font-size', headerSize + 'px')
          .attr('text-anchor', 'middle')
          .attr('transform', 'translate(' + (psize.w * 0.5) + ',' + (psize.h * 0.5 + txtSize * 0.3) + ')')
          .style('pointer-events', 'none')
      })
      let merge = current.merge(enter)
      merge.each(function (d, i) {
