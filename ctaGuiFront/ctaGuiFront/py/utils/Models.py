...
def initUsers():
  ...
  for n_user_now in range(len(user_passes)):
    my_model = MyModel(userId=user_passes[n_user_now][0],passwd=user_passes[n_user_now][1],groups=user_passes[n_user_now][2])
...
def get_groups(user_id, request):
  ...
  if db_lookup != None: return (db_lookup.groups).split(";")
  else:                return []
