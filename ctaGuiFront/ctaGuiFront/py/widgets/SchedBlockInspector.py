...
class SchedBlockInspector():
    ...
    def get_data(self):
        ...
        time_of_night_date = {
            ...
            "date_end": (datetime(2018, 9, 16, 21, 30) + timedelta(seconds=int(SchedBlockInspector.time_of_night['end']))).strftime('%Y-%m-%d %H:%M:%S'),
            "date_now": (datetime(2018, 9, 16, 21, 30) + timedelta(seconds=int(SchedBlockInspector.time_of_night['now']))).strftime('%Y-%m-%d %H:%M:%S'),
