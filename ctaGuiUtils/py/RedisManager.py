...
class RedisBase(object):
    ...
    def zAdd(self, name=None, score=None, data=None,
             packed=False, clip_score=None, packed_score=False):
        try:
...
class RedisManager(RedisBase):
    ...
    def set_pubsub(self, key=None, ignore_subscribe_messages=True):
        try:
            ...
            if self.log:
                self.log.error([
                    ['wr', ' - ', self.name, ' - could not do redis.set_pubsub() for ', str(key)],
                    ['r', ' '+str(sys.exc_info())]
                ])
    ...
    def get_pubsub(self, key=None, timeout=3600, packed=False):
        ...
        try:
            ...
            if self.log:
                self.log.error([
                    ['wr', ' - ', self.name, ' - could not do redis.get_pubsub() for ', str(key)],
                    ['r', ' '+str(sys.exc_info())]
                ])
...
def unpack_object(data_in, log=None):
    try:
        ...
        if log:
            log.error([
                ['wr', ' - ', self.name, ' - could not do unpack_object() for ', str(data_in)],
                ['r', ' '+str(sys.exc_info())]
            ])
        data = None
