...
window.TelescopeForm = function(opt_in) {
    ...
    function createTitle() {
        function drawHealthBar() {
            ...
            // ???????????????????????????????????????????????????????
            // DL_FIXME
            // - change this function to take threshold numbers
            // from the variables defined in window.set_tel_state_funcs
            // which is filled as part of the initial_connect event.
            // maybe use something like
            //   window.TEL_HEALTH_THRESHOLDS[window.TEL_STATES.ERROR]
            //
            // - also consider the DISCONNECTED state?
            // ???????????????????????????????????????????????????????
