...
window.removeTelescopeFromBlock = function(b, t) {
    if (b.tel_ids && b.tel_ids.indexOf(t.id) !== -1) {
        b.tel_ids.splice(b.tel_ids.indexOf(t.id), 1)
    }
    if (t.id.includes('L')) {
        let index = b.telescopes.large.ids.indexOf(t.id)
        if (index !== -1) {
            b.telescopes.large.ids.splice(index, 1)
        }
    }
    else if (t.id.includes('M')) {
        let index = b.telescopes.medium.ids.indexOf(t.id)
        if (index !== -1) {
            b.telescopes.medium.ids.splice(index, 1)
        }
    }
    else if (t.id.includes('S')) {
        let index = b.telescopes.small.ids.indexOf(t.id)
        if (index !== -1) {
            b.telescopes.small.ids.splice(index, 1)
        }
    }
...
window.extractRandomTelsFromBlock = function(blocks, teltype) {
    function canExtract(tel) {
        for (let i = 0; i < blocks.length; i++) {
            if (blocks[i].telescopes[teltype].ids.indexOf(tel) !== -1 && blocks[i].telescopes[teltype].ids.length <= blocks[i].telescopes[teltype].min) {
                return false
            }
        }
        for (let i = 0; i < blocks.length; i++) {
    ...
    for (let i = 0; i < blocks.length; i++) {
        let b = blocks[i]
        if (b.telescopes[teltype].ids.length <= b.telescopes[teltype].min) {
            continue
        }
...
window.blocksIntersect = function(b1, b2) {
    if (b1.time.start <= b2.time.start && b1.time.end >= b2.time.end) {
        return true
    }
    if (b1.time.end >= b2.time.start && b1.time.end <= b2.time.end) {
        return true
    }
    if (b1.time.start >= b2.time.start && b1.time.end <= b2.time.end) {
        return true
    }
    if (b1.time.start <= b2.time.end && b1.time.end >= b2.time.end) {
        return true
    }
    return false
}
...
window.clusterBlocksByTime = function(blocks) {
    ...
    for (let i = 0; i < blocks.length; i++) {
        for (let j = 0; j < clusters.length; j++) {
            ...
            if (inter) {
                clusters[j].push(blocks[i])
            }
        }
    }
    for (let i = clusters.length - 1; i >= 0; i--) {
        for (let j = i - 1; j >= 0; j--) {
            if (checkDuplicata(clusters[i], clusters[j])) {
                clusters.splice(i, 1)
            }
            break
        }
