...
class PlotsDash(BaseWidget):
    ...
    def __init__(self, widget_id=None, sm=None, *args, **kwargs):
        ...
        self.tel_ids = self.sm.inst_data.get_inst_ids(inst_types=self.tel_type)
    ...
    def init_tel_health(self):
        ...
        self.inst_health_sub = self.sm.inst_data.get_inst_healths()
    ...
    def check_sytem_health(self, agregate, key, row):
        ...
        # ???????????????????????????????????????????????????????
        # DL_FIXME
        # change this function to something like:
        #     return self.inst_data.get_inst_health_state(health)
        # ???????????????????????????????????????????????????????
    ...
    def agregate_tel_health(self, inst_health):
        ...
        for key in inst_health:
            if (inst_health[key]['health'] is None):
                continue
            if self.sm.inst_data.is_tel_type(key, 'LST'):
            ...
            elif self.sm.inst_data.is_tel_type(key, 'MST'):
            ...
            elif self.sm.inst_data.is_tel_type(key, 'SST'):
    ...
    def get_tel_state(self, health):
        ...
        # ???????????????????????????????????????????????????????
        # DL_FIXME
        # these conditions seem to be a bug anyway... health=58
        # is smaller than 60 and would be error, not warning ?!?!?
        # ???????????????????????????????????????????????????????
        if (health < 60):
            return 'ERROR'
        elif (health < 55):
